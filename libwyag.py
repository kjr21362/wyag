...
def repo_create(path):
    ...
    # First, we want to make sure the path either doesn't exist
    # or is an empty dir.
...
def gitignore_parse1(raw):
    # This parser returns a pair: the pattern itself, and a boolean
    # to indicate if files matching the pattern should be excluded (True) or included (False)
    raw = raw.strip()
...
def ls_tree(repo, ref, recursive=None, prefix=""):
    ...
    for item in obj.items:
        ...
        match type:  # Determine the type
            ...
            case _: raise Exception("Weired tree leaf mode {}".format(item.mode))
...
def index_read(repo):
    ...
    for i in range(0, count):
        ...
        # Length of the name.  This is stored on 12 bits, so max
        ...
        if name_length < 0xFFF:
            ...
            # This probably wasn't tested enough.
